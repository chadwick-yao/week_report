...
记录一下部署遇到的问题，这个问题是造成部署卡顿除因为inference很慢之外的主要原因。经过实验发现，当前部署如果预测出7步，在执行完之后，理应下次循环执行`get_obs`函数的时候，获取到的应该是当前时刻以及前几时刻的观测数据，而不是之后。就是因为这个现象，导致给机械臂规划动作的法时候，需要等待多个时刻才开始执行下一组动作序列。

## 2024/2/23

机械臂部署问题解决/30hz收集的数据进行训练的结果非常的差；

- Consistency Model/Distillation: 蒸馏算法的思想，就是利用一个预训练好的模型，将它预测的结果作为一个软标签，用作之后的训练。
- Expressive Diffusion Model: 一个关键$f_{\theta}(x,t)=c_{skip}x+c_{out}F_{\theta}(x,t)$建模。
- Progressive Distillation: 思想很简单，类似于二分法，举个例子，希望能够把一个需要16步进行还原的sampler，在不降低其输出质量的情况下，能够让模型只需要8步，然后以此类推。
